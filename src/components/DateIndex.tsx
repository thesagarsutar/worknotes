      ...
      <div className="date-index relative">
        {dates.map((date, index) => (
          ...
          >
            <div 
              className={cn(
                "date-line w-4 h-0.5 my-3 cursor-pointer transition-all duration-200",
                isToday(date) ? "bg-[#222222]" : "bg-[#C8C8C9]",
                "hover:bg-[#9b87f5] hover:w-6"
              )}
            />
          </div>
        ))}
        ...
        {hoveredDate && (
          <div 
            className="absolute left-8 bg-white dark:bg-gray-800 px-2 py-1 rounded shadow-md text-sm whitespace-nowrap"
